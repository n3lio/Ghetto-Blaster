...
function resolveSmartPlaylist(genreMatch, library) {
  if (!Array.isArray(genreMatch) || !Array.isArray(library)) return [];
  const lower = genreMatch.map(g => String(g).toLowerCase());
  return library
    .filter(t => t && t.genre && lower.some(m => t.genre.toLowerCase().includes(m)))
    .map(t => t.id);
}
...
function resolvePlaylistTracks(playlist, library) {
  ...
  if (playlist.type === 'smart' && playlist.genreMatch) {
    return resolveSmartPlaylist(playlist.genreMatch, library);
  }
